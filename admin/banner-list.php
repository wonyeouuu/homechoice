...
Routine\Permission::checkPermission ( 'SBN', true );
$page_title = "首頁Banner材質列表";
...
// Config
$config = array (
		'tblName' => 'banner_index',
		'pk' => 'bId',
		'listUrl' => $_SERVER ['PHP_SELF'],
		'title' => $page_title,
		'editUrl' => 'banner-edit.php',
		'initWhere' => ' where 1 ',
		'initSort' => ' order by a.`sort`  ',
		'initDesc' => ' asc',
		'cookieName' => 'LP_banner_index',
		'editPermission' => 'EBN',
		'sortable' => true,
		'sortField' => 'sort',
		'sortCondition' => '',
...
// 		$row['show'] = \Web\Colortable::getShow($row);
		if(!empty($row['pic']) && is_file(Routine\FileField::getImageAbs($row['pic'], 'banner', $row))){
			$row['picImg'] = '<img src="'.FileField::getImageRel($row['pic'], 'banner', $row).'" width="120" />';
		}
		$row['type'] = Model\Banner\Get::$types[$row['type']]['name'];
		$pData [] = $row;
	}
} else
	$pData = array ();
								...
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											...
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=pic">圖片</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=type">連結種類</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=link">外部連結</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=status">狀態</a></th>
