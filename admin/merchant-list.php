...
$config = array (
		...
		'title' => $page_title,
		'editUrl' => 'merchant-edit.php',
		'initWhere' => ' where 1 ',
		'initSort' => ' order by a.`mId`  ',
		'initDesc' => ' desc',
		'cookieName' => 'lp_merchant',
		'editPermission' => 'EOD',
		'sortable' => false,
...
$page = Routine\JCookie::getCache('page');
...
switch ($action) {
	case 'update':
		$sql = 'update update_time set updateTime = NOW() where type = "merchantList"';
		$db->exec($sql);
...
if ($paginate->result ['total_num'] > 0) {
	...
	$sloganOptions = Model\Merchant\Get::$slogans;
	$statusOptions=array('0'=>array('name'=>'隱藏'), '1'=>array('name'=>'顯示'), '2'=>array('name'=>'顯示於首頁'));
	while ( ( $row = $pre->fetch ( PDO::FETCH_ASSOC ) ) !== false ) {
		$row['sloganText'] = $sloganOptions[$row['slogan']]['name'];
		$row['statusText'] = $statusOptions[$row['status']]['name'];
		$pData [] = $row;
	}
} else
	$pData = array ();
	...
	<div id="content">
		...
		<div class="row">
			...
			<div style="float: right; margin: 10px 20px;">
				...
				<a class="btn btn-primary" href="<?php echo $config['listUrl']?>?action=update">
					<i class="fa fa-lg fa-fw fa-plus"></i>更新至APP
				</a>
			</div>
			<?php endif;?>
		</div>
					...
					<div class="jarviswidget jarviswidget-color-blueDark" id="widget-account-list" data-widget-deletebutton>
						...
						<div>
							...
							<div class="jarviswidget-editbox">
								<form action="<?php echo $_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data" name="sform">
									名稱:
							...
							<div class="widget-body list-body">
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=name">名稱</a></th>
											<th>簡述</th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=endDate">期限</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=slogan" <?php if($key == 'lastname') echo 'style="color:blue;"'?>>SLOGAN</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=status" <?php if($key == 'lastname') echo 'style="color:blue;"'?>>狀態</a></th>
