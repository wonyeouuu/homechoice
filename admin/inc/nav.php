...
<?php //echo '<pre>';print_r($page_nav);exit;?><aside id="left-panel">

	<!-- User info -->
	<div class="login-info">
		<span> <!-- User image size is adjusted inside CSS, it should stay as it --> <a href="javascript:void(0);" id="show-shortcut">
				<span>
							<?php echo $_SESSION['BkName']?>
						</span>
	...
	<nav>
		...
		<ul>
					...
					foreach ( $page_nav as $key => $nav_item ) {
						...
						if (! isset ( $nav_item ['permission'] ) || ( isset ( $nav_item ['permission'] ) && Routine\Permission::checkPermission ( $nav_item ['permission'], false ) )) {
							echo $nav_item['title'];
							...
							$nav_title = isset ( $nav_item ["title"] ) ? $nav_item ["title"] : "(No NameP)";
							...
							if (isset ( $nav_item ["sub"] ) && $nav_item ["sub"]) {
								// print_r($nav_item['sub']);
								$sub_html = process_sub_nav ( $nav_item ['sub'] );
								if (! $sub_html) {
									continue;
								}
								$nav_htm .= $sub_html;
						...
						if (isset ( $nav_item ["sub"] ) && $nav_item ["sub"]) {
							...
							foreach ( $nav_item as $key => $sub_item ) {
								if (isset ( $sub_item ['permission'] )) {
									...
									if (! Routine\Permission::checkPermission ( $sub_item ['permission'], false )) {
// echo '沒有權限';
// print_r($_SESSION[WebCode.'bkpermission']);
										continue;
									}
								}
