...
Routine\Permission::checkPermission ( 'SPA', true );
$page_title = "加購商品列表";
...
// Config
$config = array (
		'tblName' => 'av_accessory',
		'pk' => 'accessory_id',
		'listUrl' => $_SERVER ['PHP_SELF'],
		'title' => $page_title,
		'editUrl' => 'accessory-edit.php',
		'initWhere' => ' where 1 ',
		'initSort' => ' order by a.`accessory_id`  ',
		'initDesc' => ' asc',
		'cookieName' => 'av_accessory',
		'editPermission' => 'EPC',
		'sortable' => false,
...
if ($paginate->result ['total_num'] > 0) {
	...
	while ( ( $row = $pre->fetch ( PDO::FETCH_ASSOC ) ) !== false ) {
		$row['show'] = '<img height="100" src="'.\Routine\FileField::getImgRel('accessory', 'image', $row['image']).'" />';
		$pData [] = $row;
	}
} else
	$pData = array ();
								...
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											...
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=price" <?php if($key == 'price') echo 'style="color:blue;"'?>>價格</a></th>
											<th>圖片</th>
