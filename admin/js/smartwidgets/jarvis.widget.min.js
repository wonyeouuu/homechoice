(function($, window, document, undefined) {
	...
	Plugin.prototype = {
	    ...
	    _getPastTimestamp : function(t) {
		    ...
		    var format = self.o.timestampFormat.replace(/%d%/g, tsDay).replace(/%m%/g, tsMonth).replace(/%y%/g, tsYear).replace(/%h%/g, tsHours).replace(/%i%/g, tsMinutes)
	    ...
	    init : function() {
		    ...
		    if (storage && self.o.localStorage && getKeySettings) {
			    ...
			    for ( var key in jsonSettings.widget) {
				    ...
				    if (jsonSettings.widget[key].style) {
					    widgetId.removeClassPrefix("jarviswidget-color-").addClass(jsonSettings.widget[key].style).attr("data-widget-attstyle",
		            ...
		            .each(function() {
			            ...
			            if (!thisHeader.parent().attr("role")) {
				            ...
				            if (self.o.deleteButton === true && tWidget.data("widget-deletebutton") === undefined) {
					            var deleteBtn = '<a href="javascript:void(0);" class="button-icon jarviswidget-delete-btn" rel="tooltip" title="Delete" data-placement="bottom"><i class="'
					                    + self.o.deleteClass + '"></i></a>'
				            } else {
					            deleteBtn = ""
				            }
				            if (self.o.editButton === true && tWidget.data("widget-editbutton") === undefined) {
					            var editBtn = '<a href="javascript:void(0);" class="button-icon jarviswidget-edit-btn" rel="tooltip" title="搜尋" data-placement="bottom"><i class="'
					                    + self.editClass[0] + '"></i>搜尋</a>'
				            } else {
					            editBtn = ""
				            }
				            if (self.o.fullscreenButton === true && tWidget.data("widget-fullscreenbutton") === undefined) {
					            var fullscreenBtn = '<a href="javascript:void(0);" class="button-icon jarviswidget-fullscreen-btn" rel="tooltip" title="全螢幕" data-placement="bottom"><i class="'
				            ...
				            if (self.o.toggleButton === true && tWidget.data("widget-togglebutton") === undefined) {
					            ...
					            var toggleBtn = '<a href="#" class="button-icon jarviswidget-toggle-btn" rel="tooltip" title="隱藏" data-placement="bottom"><i class="'
					                    + toggleSettings + '"></i></a>'
				            } else {
					            toggleBtn = ""
				            }
				            if (self.o.refreshButton === true && tWidget.data("widget-refreshbutton") != false && tWidget.data("widget-load")) {
					            var refreshBtn = '<a href="#" class="button-icon jarviswidget-refresh-btn" data-loading-text="&nbsp;&nbsp;Loading...&nbsp;" rel="tooltip" title="Refresh" data-placement="bottom"><i class="'
					                    + self.o.refreshButtonClass + '"></i></a>'
				            } else {
					            refreshBtn = ""
				            }
				            var formatButtons = self.o.buttonOrder.replace(/%refresh%/g, refreshBtn).replace(/%delete%/g, deleteBtn).replace(/%custom%/g, customBtn).replace(
				                    /%fullscreen%/g, fullscreenBtn).replace(/%edit%/g, editBtn).replace(/%toggle%/g, toggleBtn);
		    ...
		    self.obj.find("[data-widget-load]")
		            .each(
		                    function() {
			                    var thisItem = $(this), thisItemHeader = thisItem.children(), pathToFile = thisItem.data("widget-load"), reloadTime = thisItem
			                            .data("widget-refresh") * 1000, ajaxLoader = thisItem.children();
	    ...
	    _clickEvents : function() {
		    ...
		    self.widget.on(clickEvent, ".jarviswidget-toggle-btn", function(e) {
			    ...
			    if (pWidget.hasClass("jarviswidget-collapsed")) {
				    tWidget.children().removeClass(self.toggleClass[1]).addClass(self.toggleClass[0]).parents(self.o.widgets).removeClass("jarviswidget-collapsed").children(
				            "[role=content]").slideDown(self.o.toggleSpeed, function() {
					    self._saveSettingsWidget()
				    })
			    } else {
				    tWidget.children().removeClass(self.toggleClass[0]).addClass(self.toggleClass[1]).parents(self.o.widgets).addClass("jarviswidget-collapsed").children(
				            "[role=content]").slideUp(self.o.toggleSpeed, function() {
					    self._saveSettingsWidget()
				    })
			    }
		    ...
		    self.widget.on(clickEvent, ".jarviswidget-fullscreen-btn", function(e) {
			    ...
			    if ($("#jarviswidget-fullscreen-mode").length) {
				    ...
				    thisWidget.unwrap("<div>").children("div").removeAttr("style").end().find(".jarviswidget-fullscreen-btn").children().removeClass(self.fullscreenClass[1])
				            .addClass(self.fullscreenClass[0]).parents(self.pwCtrls).children("a").show();
				    if (thisWidgetContent.hasClass("jarviswidget-visible")) {
					    thisWidgetContent.hide().removeClass("jarviswidget-visible")
				    }
			    } else {
				    $("body").addClass("nooverflow");
				    thisWidget.wrap('<div id="jarviswidget-fullscreen-mode"/>').parent().find(".jarviswidget-fullscreen-btn").children().removeClass(self.fullscreenClass[0])
				            .addClass(self.fullscreenClass[1]).parents(self.pwCtrls).children("a:not(.jarviswidget-fullscreen-btn)").hide();
