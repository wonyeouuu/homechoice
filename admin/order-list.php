...
Routine\Permission::checkPermission ( 'SOD', true );
$page_title = "訂單列表";
...
$config = array (
		...
		'title' => $page_title,
		'editUrl' => 'order-edit.php',
		'initWhere' => ' where 1 ',
		'initSort' => ' order by a.`add_date`  ',
		'initDesc' => ' desc',
		'cookieName' => 'av_order',
		'editPermission' => 'EPC',
		'sortable' => false,
...
if ($paginate->result ['total_num'] > 0) {
	...
	while ( ( $row = $pre->fetch ( PDO::FETCH_ASSOC ) ) !== false ) {
		$row = Web\Orders::convertOrdersInfor ( $row );
		$pData [] = $row;
	}
} else
	$pData = array ();
						...
						<div>
							...
							<div class="jarviswidget-editbox">
								<form action="<?=$_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data" name="sform">
									訂購者: <input name="search[name]" value="<?php echo $Grid->searchData['name'];?>" class="input" size="8" /> 訂購日期: <input name="search[start_date]"
										value="<?php echo $Grid->searchData['start_date'];?>" class="datepicker" size="8" />~ <input name="search[end_date]" value="<?php echo $Grid->searchData['end_date'];?>" class="datepicker"
										size="8" />
							...
							<div class="widget-body list-body">
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=order_id" <?php if($key == 'catagories_id') echo 'style="color:blue;"'?>>訂單編號</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=name" <?php if($key == 'name') echo 'style="color:blue;"'?>>訂購者/收件者</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=total_price" <?php if($key == 'brand_id') echo 'style="color:blue;"'?>>總金額</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=status_text" <?php if($key == 'price') echo 'style="color:blue;"'?>>訂單狀態</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=payment_status_text" <?php if($key == 'discount') echo 'style="color:blue;"'?>>付款狀態</a></th>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=add_date" <?php if($key == 'online_date') echo 'style="color:blue;"'?>>訂單日期</a></th>
									...
									<tbody>
										...
										<tr>
											...
											<td><?php echo $row['name']?> / <?php echo $row['invoice_name']?></td>
											<td><?php echo $row['total_price']?></td>
											<td><?php echo $row['status_text']?></td>
											<td><?php echo $row['payment_status_text']?></td>
