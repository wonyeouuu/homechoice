...
Routine\Permission::checkPermission ( 'SCR', true );
$page_title = "點擊記錄";
...
$clickCodes=\Model\Click\Code::$codes;

$matchCodes=$clickCodes;
$matchKeys=array_keys($matchCodes);
// Config
$config = array (
		'tblName' => 'click_record',
		'pk' => 'crId',
		'listUrl' => $_SERVER ['PHP_SELF'],
		'title' => $page_title,
		'editUrl' => 'install-edit.php',
		'initWhere' => ' where 1 ',
		'initGroup'=>' group by a.addDate, a.code',
		'initSort' => ' order by a.`addDate`  ',
		'initDesc' => ' desc',
		'cookieName' => 'click_record',
		'editPermission' => 'MSR',
		'sortable' => false,
...
$Grid->whereSql .= ' and a.code in ('.implode(',', $matchKeys).')';

$count_sql = "select count(*) from " . $config ['tblName'] . " a   " . $Grid->whereSql . $Grid->groupSql;
$query_sql = "select a.addDate, a.code, COUNT(a.crId) codeCount
...
// 		$pData [] = $row;
		if(!isset($list[$row['addDate']])){
			$list[$row['addDate']]=array();
		}
		$list[$row['addDate']][$row['code']]=$row['codeCount'];
	}
} else
	$pData = array ();
							...
							<div class="widget-body list-body">
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											<th>日期</th>
											<?php foreach ($matchCodes as $code => $codeRow){?>
											<th><a href="<?=$_SERVER['PHP_SELF']?>?sort=addDate" <?php if($key == 'addDate') echo 'style="color:blue;"'?>><?php echo $codeRow['name']?></a></th>
											<?php }?>
											<th style="width: auto;">操作</th>
										</tr>
									</thead>
									<tbody>
										<?php foreach ($list as $date => $row){?>
										<tr>
											<td><?php echo $date?></td>
											<?php foreach ($matchCodes as $code => $codeRow){?>
											<td><?php echo isset($row[$code])?$row[$code]:0?></td>
											<?php }?>
											<td>
												<?php if(Routine\Permission::checkPermission($config['editPermission'], false)):?>
