...
Routine\Permission::checkPermission ( 'SCP', true );
$page_title = "特店列表";
...
$config = array (
		...
		'title' => $page_title,
		'editUrl' => 'coupon-edit.php',
		'initWhere' => ' where 1 ',
		'initSort' => ' order by a.`cpId`  ',
		'initDesc' => ' desc',
		'cookieName' => 'lp_coupon',
		'editPermission' => 'ECP',
		'sortable' => false,
...
switch ($action) {
	case 'update':
		$sql = 'update update_time set updateTime = NOW() where type = "couponList"';
		$db->exec($sql);
...
if ($paginate->result ['total_num'] > 0) {
	...
	$sloganOptions = Model\Coupon\Get::$slogans;
